...
public class RegisterEmailVerificationService {
    ...
    public RegisterEmailVerificationService(
            UserRepository userRepository,
            JavaMailSender mailSender,
            @Value("${spring.mail.username:}") String mailUsername,
            @Value("${smartnote.auth.register-code.expire-minutes:10}") long expireMinutes,
            @Value("${smartnote.auth.register-code.resend-seconds:60}") int resendSeconds,
            @Value("${smartnote.auth.register-code.subject:SmartNote 注册验证码}") String codeSubject
    ) {
        this.userRepository = userRepository;
        this.mailSender = mailSender;
        this.mailUsername = normalizeEmail(mailUsername);
